...
namespace StudentFinanaceBudgetingAssisatant
{
    public partial class Main : Form
    {
        ...
        string[] CategorieIn = { "Loan-StudentFinance", "Grant-StudentFinance", "Bursary", "Job" };
		...
		public struct Configuration
		{
			public int CurrentYear;
				// Currently the most recently edited. Some way of improving
				// this without adding extra gui elements?
...
CBRepeatFreqIn.Text);
        ...
        private int GetTerm()
        {
            ...
            if (Today < RC.YearInfo[RC.CurrentYear].T2.TermStart ) return 0;
            else if (Today < RC.YearInfo[RC.CurrentYear].T3.TermStart) return 1;
            else return 2;
        }

        private int GetTerm(DateTime Given)
        {
            if (Given < RC.YearInfo[RC.CurrentYear].T2.TermStart) return 0;
            else if (Given < RC.YearInfo[RC.CurrentYear].T3.TermStart) return 1;
            else return 2;
        }
        #endregion
